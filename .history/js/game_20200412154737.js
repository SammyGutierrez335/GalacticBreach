    ...
    if (this.enemies.length > 0) {
      if (this.enemies.length <= this.maxEnemies) {
        ...
        for (let i = 0; i < this.enemies.length; i++) {
          ...
          if (enemy.hit[0]) {
            enemy.despawning += 1
            if (enemy.despawning === 16) {
            this.remove(enemy, enemy.hit[1])
            }
            
            let prevSrc = enemy.enemyImage.src
            enemy.enemyImage = new Image()
            enemy.enemyImage.src = "assets/attackers/explosions/explosion-animation-sprite-blast.jpg"
          ...
          if (enemy.despawning) {
            console.log("enemyX", enemy.x, "enemy.Y", enemy.y)
            ctx.drawImage(enemyImage,
              0, 0, 500, 500,
              enemy.x - enemy.speed, enemy.y + 10, 125, 125)
            } else {
              ctx.drawImage(enemyImage,
